...
func TestCreateCustomer(t *testing.T) {
	...
	router.POST("/customers", func(c *gin.Context) {
		status, _, err := createCustomer(mockDB, c)
		c.JSON(status, err)
	})
...
func TestGetCustomer(t *testing.T) {
	...
	router.GET("/customers/:customerId", func(c *gin.Context) {
		status, _, err := getCustomer(mockDB, c)
		c.JSON(status, err)
	})
...
func TestUpdateCustomer(t *testing.T) {
	...
	// When
	router := gin.New()
	router.PUT("/customers/:customerId", func(c *gin.Context) {
		status, _, err := updateCustomer(mockDB, c)
		c.JSON(status, err)
	})
